...
2. Satisfaction signals (time → extended to interval form):
...
* For each predicate **µ** over the tube **[ỹ](t)**, Unitary boolean label each time slice \[tj, tj+1] as:
  **1** if [ỹ]([tj, tj+1]) ⊂ Xµ, **0** if [ỹ]([tj, tj+1]) ∩ Xµ = ∅, **[0,1]** otherwise.
